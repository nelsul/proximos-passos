...
func (r *GroupRepository) ListMembers(ctx context.Context, groupID int, limit, offset int) ([]entity.GroupMember, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT gm.group_id, gm.user_id, u.public_id, gm.role, gm.accepted_by_id,
...
func (r *GroupRepository) CountMembers(ctx context.Context, groupID int) (int, error) {
	var count int
	err := r.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM group_members WHERE group_id = $1 AND is_active = true`,
		groupID,
	).Scan(&count)
	return count, err
}
