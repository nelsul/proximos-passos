...
import { getExamDetails, type ExamDetailResponse } from "@/lib/exams";
...
/** Return the set of IDs that need to be expanded (i.e. ancestors of highlighted topics) */
...
export default function ExamDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [examDetail, topicsRes] = await Promise.all([
  ...
  // Filter tree to only show branches that contain highlighted topics
  ...
  if (!detail) {
    return (
      <div className="rounded-lg border border-surface-border bg-surface p-8 text-center">
        <p className="text-muted">{t("ERROR_EXAM_NOT_FOUND")}</p>
      </div>
    );
  }
  ...
  return (
    <div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-2xl font-bold text-heading">
              {exam.institution.acronym} — {exam.title} ({exam.year})
            </h1>
            <p className="mt-1 text-sm text-muted">{exam.institution.name}</p>
            {exam.description && (
              <p className="mt-2 text-sm text-body">{exam.description}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <button
          onClick={() =>
            router.push(`/${locale}/dashboard/questions?exam_id=${exam.id}`)
          }
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-muted">{t("EXAM_DETAIL_QUESTIONS")}</p>
          </div>
        </button>
        ...
        <div className="flex items-center gap-4 rounded-xl border border-surface-border bg-surface p-5">
          ...
          <div>
            ...
            <p className="text-sm text-muted">{t("EXAM_DETAIL_TOPICS")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-surface-border bg-surface p-5">
        <h2 className="mb-4 text-lg font-semibold text-heading">
          {t("EXAM_DETAIL_TOPIC_TREE")}
        </h2>

        {filteredTree.length === 0 ? (
          <div className="py-8 text-center">
            <Layers className="mx-auto mb-3 h-8 w-8 text-muted" />
            <p className="text-sm text-muted">{t("EXAM_DETAIL_NO_TOPICS")}</p>
          </div>
        ) : (
