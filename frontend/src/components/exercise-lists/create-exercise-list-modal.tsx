...
import { createExerciseList } from "@/lib/open-exercise-lists";
import { ApiRequestError } from "@/lib/api";
import { InputField } from "@/components/ui/input-field";
import { Button } from "@/components/ui/button";
import { TopicPickerModal } from "@/components/handouts/topic-picker-modal";

interface CreateExerciseListModalProps {
  onClose: () => void;
  onCreated: () => void;
}

export function CreateExerciseListModal({
  onClose,
  onCreated,
}: CreateExerciseListModalProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!file && !fileURL.trim()) {
      setError(t("EXERCISE_LIST_SOURCE_REQUIRED"));
      return;
    }
    ...
    try {
      ...
      if (fileURL.trim()) formData.append("file_url", fileURL.trim());
      if (file) formData.append("file", file);
      selectedTopics.forEach((t) => formData.append("topic_ids", t.id));

      await createExerciseList(formData);
      onCreated();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              {file ? file.name : t("EXERCISE_LIST_FILE_CHOOSE")}
            </button>
          </div>
